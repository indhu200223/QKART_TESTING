...
public class QKART_Tests {
     ...
     */
         @Test(groups ={"Sanity_test"},description = "Verifying Registeration happens successfully.",priority=1)
    ...
    @Test(groups ={"Sanity_test"} ,description = " Verifying Re-registering an already registered user fails",priority=2)
    public void TestCase02() throws InterruptedException {
        ...
        Assert.assertFalse(status, "Registeration is happening with the existing user name");
   ...
   @Test(groups ={"Sanity_test"},description = "Verifying functionality of search text box.",priority = 3)
    public void TestCase03() throws InterruptedException {
        ...
        Assert.assertTrue(status,"Unable to find the given product");
        // if (!status) {
     ...
     * expected
     */
    @Test(groups={" Regression_Test"},description = "Verifying the existence of size chart for certain items and validate contents of size chart",priority=4)
    public void TestCase04() throws InterruptedException {
        ...
        for (WebElement webElement : searchResults) {
            ...
            if (result.verifySizeChartExists()) {
                ...
                Assert.assertTrue(status,"Size chart dropdown exists");
                ...
                if (result.openSizechart()) {
                   ...
                   Assert.assertTrue(status,"Size chart has been closed");
     ...
     * working correctly
     */
    @Test(groups ={"Sanity_test"},description = "Verifying that a new user can add multiple products in to the cart and Checkout.",priority = 5)
    ...
    public void TestCase05(String YONEX,String Tan,String Address) throws InterruptedException {
        ...
        // Register a new user
        status = registration.registerUser("testUser", "abc@123", true);
        Assert.assertTrue(status,"Registeration is unsuccessful");
        // if (!status) {
        ...
        Assert.assertTrue(status,"Product has been found");
        homePage.addProductToCart(YONEX); //"YONEX Smash Badminton Racquet"
        status = homePage.searchForProduct(Tan);
        Assert.assertTrue(status,"Product has been found");
        homePage.addProductToCart(Tan); //Tan Leatherette Weekender Duffle
        ...
        Assert.assertTrue(status,"Current url doesn't ends with /thanks");
     ...
     */
     @Test(groups={" Regression_Test"},description = "Verifying that the contents of the cart can be edited.",priority=6)
    ...
    public void TestCase06(String Xtend,String Yarine) throws InterruptedException {
        ...
        Assert.assertTrue(status,"Product has been found");
        homePage.addProductToCart(Xtend); //"Xtend Smart Watch"

        status = homePage.searchForProduct(Yarine);
        Assert.assertTrue(status,"Product has been found");
        homePage.addProductToCart(Yarine); //"Yarine Floor Lamp"
    ...
    @Test(groups ={"Sanity_test"},description = " Verifying that insufficient balance error is thrown when the wallet balance is not enough.",priority = 7)
    @Parameters ({"Stylecon","quantity","Address_1"})
    public void TestCase07(String Stylecon,int quantity,String Address_1) throws InterruptedException {
        Boolean status;
        ...
        registration.navigateToRegisterPage();
        status = registration.registerUser("testUser", "abc@123", true);
        Assert.assertTrue(status,"User can't be registered");
        // if (!status) {
        ...
        homePage.addProductToCart(Stylecon); //Stylecon 9 Seater RHS Sofa Set 

        homePage.changeProductQuantityinCart(Stylecon, quantity); //10

        homePage.clickCheckout();

        Checkout checkoutPage = new Checkout(driver);
        checkoutPage.addNewAddress(Address_1);
        checkoutPage.selectAddress(Address_1); //"Addr line 1 addr Line 2 addr line 3"

        checkoutPage.placeOrder();
        Thread.sleep(3000);

        status = checkoutPage.verifyInsufficientBalanceMessage();
        Assert.assertTrue(status,"Error in displaying insufficient balance message");
    ...
    @Test(groups={" Regression_Test"},description = "Verifying that a product added to a cart is available when a new tab is added.",priority = 8)
    ...
    @Test(groups={" Regression_Test"},description ="Verifying that privacy policy and about us links are working fine.",priority = 9)
    public void TestCase09() throws InterruptedException {
        ...
        Assert.assertTrue(status,"unable to verify the URL");
        ...
        Assert.assertTrue(status,"unable to verify the text");
        // if (!status) {
    ...
    @Test(groups={" Regression_Test"},description = "Verifying that the contact us dialog works fine.",priority = 10)
    ...
    @Test(groups ={"Sanity_test"},description = "Ensuring that the Advertisement Links on the QKART page are clickable.",priority = 11)
